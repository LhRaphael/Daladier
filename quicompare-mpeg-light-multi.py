...
# -*- coding: utf-8 -*-
"""
QUICompare-mpeg v4.1.5 - LIGHT MULTI

Modo leve multifractal:
 - Pode usar ffmpeg (streaming) ou curl (HTTP download)
 - Gera séries temporais de vazão e latência
 - Calcula métricas mono + multifractais:
      - Hurst (R/S)
      - Alfa tail shape (Hill)
      - Hill estimator
      - FD via madogram
      - Whittle estimator
      - MFDFA (tau_min, tau_max)
      - Lyapunov exponent
      - Classificação ETrap (mice / moderated / chaotic / hard)
 - Salva sumário formato B + JSON
 - Organiza tudo em pastas por execução: quicompare-mpeg-light-multi-<timestamp>/
"""
...
def main():
    parser = argparse.ArgumentParser(description="QUICompare-mpeg v4.1.5 LIGHT MULTI")
    ...
    for d in [series_dir, pcap_dir, meta_dir, logs_dir, fractal_dir]:
    ...
    if args.traffic_mode == "ffmpeg":
        ...
        remote_pcap = f"/tmp/quicompare_light_multi_{ts}.pcap"
        ...
        while True:
            ...
            progress_bar(elapsed, args.duration, prefix="[LIGHT MULTI]", enabled=show_progress)
            if elapsed >= args.duration:
                break
            time.sleep(1)
        if show_progress:
    ...
    # Multifractal
    tau_min, tau_max = simple_mfdfa(thr)
    lyap = estimate_lyapunov(thr)
    ...
    # -------------------------------------------------------------
    summary = {
        "version": "QUICompare LIGHT MULTI v4.1.5",
        ...
        "mfdfa_tau_min": tau_min,
        "mfdfa_tau_max": tau_max,
        "lyapunov": lyap,
    ...
    with open(txt, "w") as f:
        f.write("--- QUICompare LIGHT MULTI ---\n")
    ...
    summary["saved_to"] = js
    print_summary("QUICompare LIGHT MULTI", summary)

...
# -*- coding: utf-8 -*-
"""
QUICompare-mpeg v4.1.5 - SUPERLIGHT

Modo leve para medições ativas de tráfego:
 - Pode usar ffmpeg (streaming) ou curl (HTTP download)
 - Gera séries temporais de vazão e latência
 - Calcula métricas fractais básicas (Hurst, alfa-tail shape, ETrap)
 - Salva sumário formato B e JSON
 - Organiza tudo em pastas por execução: quicompare-mpeg-superlight-<timestamp>/
"""
...
def main():
    parser = argparse.ArgumentParser(description="QUICompare-mpeg v4.1.5 SUPERLIGHT")
    ...
    for d in [series_dir, pcap_dir, meta_dir, logs_dir]:
    ...
    if args.traffic_mode == "ffmpeg":
        ...
        if not password:
            if not args.quiet:
                print("🔐 Senha SSH do DUT (Dica: defina export DUT_PASSWORD='senha' para automatizar):")
            password = getpass.getpass("> ")
        else:
            if not args.quiet:
                print("🔐 Usando senha SSH definida na variável de ambiente DUT_PASSWORD.")

        ssh = ssh_connect(args.dut_host, args.dut_user, password)

        remote_pcap = f"/tmp/quicompare_superlight_{ts}.pcap"
        ...
        while True:
            ...
            progress_bar(elapsed, args.duration, enabled=show_progress)
            if elapsed >= args.duration:
                break
            time.sleep(1)
        if show_progress:
    ...
    # -------------------------------------------------------------
    summary = {
        "version": "QUICompare SUPERLIGHT v4.1.5",
    ...
    summary["saved_to"] = js
    print_summary("QUICompare SUPERLIGHT", summary)

...
# -*- coding: utf-8 -*-
"""
QUICompare-mpeg v4.1.5 - MULTI FULL (com plots)

Modo multifractal completo:
 ...
 - Organiza tudo em pastas por execução: quicompare-mpeg-multi-plots-<timestamp>/
"""
...
def main():
    parser = argparse.ArgumentParser(description="QUICompare-mpeg v4.1.5 MULTI FULL (plots)")
    ...
    if args.traffic_mode == "ffmpeg":
        ...
        remote_pcap = f"/tmp/quicompare_multi_full_{ts}.pcap"
        ...
        while True:
            ...
            progress_bar(elapsed, args.duration, prefix="[MULTI FULL]", enabled=show_progress)
            if elapsed >= args.duration:
                break
            time.sleep(1)
        if show_progress:
    ...
    # -------------------------------------------------------------
    summary = {
        "version": "QUICompare MULTI FULL v4.1.5",
    ...
    summary["saved_to"] = js
    print_summary("QUICompare MULTI FULL", summary)

...
# -*- coding: utf-8 -*-
"""
QUICompare-mpeg v4.1.5 - LIGHT MONO

Modo leve monofractal:
 ...
 - Calcula métricas monofractais:
      ...
      - Hill estimator
      - FD via madogram
      - Whittle estimator (freq domain)
      - Classificação ETrap (mice / moderated / chaotic / hard)
 - Salva sumário formato B + JSON
 - Organiza tudo em pastas por execução: quicompare-mpeg-light-mono-<timestamp>/
"""
...
def main():
    parser = argparse.ArgumentParser(description="QUICompare-mpeg v4.1.5 LIGHT MONO")
    ...
    if args.traffic_mode == "ffmpeg":
        ...
        remote_pcap = f"/tmp/quicompare_light_mono_{ts}.pcap"
        ...
        while True:
            ...
            progress_bar(elapsed, args.duration, prefix="[LIGHT MONO]", enabled=show_progress)
            if elapsed >= args.duration:
                break
            time.sleep(1)
        if show_progress:
    ...
    # salva métricas fractais em JSON separado
    ...
    summary["saved_to"] = js
    print_summary("QUICompare LIGHT MONO", summary)
